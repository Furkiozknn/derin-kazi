...
var radar := false
var yaricap := Ayarlar.ISIK_YARICAP   ## Derin Mod'da 3 (Durum.isik_yaricap verir)
...
##   isik_yaricap : aracın ışığı (varsayılan ilk oyunun 5'i; Derin Mod 3)
static func ortu(uzaklik2: int, derinlik: int, kesfedildi: bool, radar_acik: bool,
		isik_yaricap := Ayarlar.ISIK_YARICAP) -> float:
	if derinlik < 0:
		return 0.0           ## gökyüzü
	var u := sqrt(float(uzaklik2))
	# Işık kenarı yumuşak: sert daire kenarı karo ızgarasında merdiven gibi duruyor.
	var isik := clampf((u - (float(isik_yaricap) - Ayarlar.ISIK_YUMUSAMA))
...
func yenile(yeni_merkez: Vector2i, radar_acik: bool, p_yaricap := Ayarlar.ISIK_YARICAP) -> void:
	merkez = yeni_merkez
	radar = radar_acik
	yaricap = p_yaricap
	queue_redraw()
...
func _draw() -> void:
	...
	for dy in range(-PENCERE_Y, PENCERE_Y + 1):
		for dx in range(-PENCERE_X, PENCERE_X + 1):
			...
			var a := ortu(dx * dx + dy * dy, h.y, dunya.kesfedildi_mi(h), radar, yaricap)
			if a <= 0.01:
				continue
